...
function PersonalRecipeCard({ recipe, onRemove, onUpdate }) {
    ...
    const [draft, setDraft] = useState({ notes: recipe.notes || "", rating: recipe.rating || 0 });
        ...
        <article className="recipe-card personal-card">
            ...
            {recipe.tags.length > 0 && (
                <div className="tag-row">
                    {recipe.tags.map((tag) => (
                        <span key={`${recipe.id}-${tag}`} className="tag-pill">{tag}</span>
                    ))}
                </div>
            )}
            ...
            {!editing && (
                <div className="card-meta">
                    ...
                    {recipe.notes && (
                        <p className="personal-notes">📝 {recipe.notes}</p>
                    )}
                </div>
            )}
            ...
            <div className="card-actions">
                <button
                    ...
                    onClick={() => { setDraft({ notes: recipe.notes || "", rating: recipe.rating || 0 }); setEditing((e) => !e); }}
                >
